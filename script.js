/* =========================================================
   ...
   ── smoother; timer‑driven fish, mystical comet tail ──
   ========================================================= */
...
/* -------- FISH (blue body, white 8‑dot trail, white outline) -------- */
class Fish {
    constructor(dir) {
        ...
        this.totalX = baseDist * (1.2 + Math.random() * 1.9);               // keep original distances
        const pxPerFr = 8 + Math.random() * 2;
        this.dur = this.totalX / pxPerFr;                                    // smooth speed
    ...
    update() {
        ...
        const slot = this.tp & 7; this.trail[slot] ||= { x: 0, y: 0 };
...
/* ---------------- COMET – mystical multicolour tail, centre 50 % land zone ---------------- */
class Comet {
    constructor() {
        ...
        const dx = this.xImpact - this.x, dy = this.waterY - this.y, speed = 18 + Math.random() * 9,
    ...
    update() {
        ...
        if (ny >= this.waterY) {
            const f = (ny - this.waterY) / this.vy;
            nx = this.x + this.vx * (1 - f); ny = this.waterY;
            splashes.push(new Splash(nx, ny, 220)); this.alive = false;
        }
        this.x = nx; this.y = ny;
        this.trail.push({ x: this.x, y: this.y }); if (this.trail.length > 25) this.trail.shift();
        return this.alive;
    }
    draw(ctx) {
        if (this.trail.length > 1) {
            ...
            g.addColorStop(0.50, '#34d399');
            g.addColorStop(0.75, '#38bdf8');
        ...
        ctx.save();
        ctx.shadowColor = 'rgba(125,211,252,0.8)'; ctx.shadowBlur = 14;
        ctx.fillStyle = '#67e8f9';
        ctx.beginPath(); ctx.arc(this.x, this.y, 3.2, 0, 2 * Math.PI); ctx.fill();
        ctx.restore();
    }
}
...
/* --------------- GLOBALS & INIT --------------- */
let canvas, ctx;
const particles = [], fishes = [], comets = [], splashes = [];
let time = 0;

/* timer‑based fish spawner (frames until next fish) */
let fishCountdown = 60 + Math.random() * 180;   // 1 – 4 seconds initial
...
function animate() {
    ...
    /* timer‑driven fish spawn */
    fishCountdown--;
    if (fishCountdown <= 0) {
        fishes.push(new Fish(Math.random() < 0.5 ? 1 : -1));
        fishCountdown = 60 + Math.random() * 180;          // 1 – 4 seconds next
    }

    /* random comet */
    if (Math.random() < 0.0001) comets.push(new Comet());
...
window.addEventListener('resize', () => {
    ...
    fishCountdown = 60 + Math.random() * 180;               // reset timer
    init();
});
